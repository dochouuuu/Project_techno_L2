...
bool test_game_default(void) {
    ...
    // Vérifie que les cases ne sont pas vides et que toutes les orientations sont valides
    ...
    // Renvoie true si toutes les cases sont remplies, orientations valides et les jeux sont identiques
        ...
        for (uint j = 0; j < DEFAULT_SIZE; j++) {
            for (direction d = NORTH; d < NB_DIRS; d++) {
                ...
                if (i == 0 && j == 0 && game_get_piece_shape(g, i, j) == ENDPOINT && d == EAST) {
                    if (!has_half_edge) {
                        printf("Erreur : ENDPOINT à (%u, %u) devrait avoir une demi-arête dans la direction %d.\n", i, j, d);
                        result = false;
                    }
                } else if (i == 1 && j ==0 && game_get_piece_shape(g, i, j) == SEGMENT && (d == EAST || d == WEST)) {
                        if (!has_half_edge) {
                            printf("Erreur : SEGMENT à (%u, %u) devrait avoir une demi-arête dans la direction %d.\n", i, j, d);
                            result = false;
                        }
    ...
    for (uint i = 0; i < DEFAULT_SIZE; i++) {
        for (uint j = 0; j < DEFAULT_SIZE; j++) {
            for (direction d = NORTH; d < NB_DIRS; d++) {
                ...
                if (has_half_edge) {
                    printf("Erreur : Jeu vide à (%u, %u) ne devrait pas avoir de demi-arête dans la direction %d.\n", i, j, d);
                    result = false;
                }
            }
        }
        ...
        for (uint j = 0; j < DEFAULT_SIZE; j++) {
            for (direction d = NORTH; d < NB_DIRS; d++) {
                ...
                if ((status == MATCH && has_half_edge == false) || (status == NOEDGE && has_half_edge == true)) {
                    printf("Erreur : à (%u, %u), direction %d\n",i, j, d);
...
bool test_game_is_well_paired(void) {
    ...
    shape shapes[5 * 5] = {
        EMPTY, EMPTY, EMPTY, EMPTY, ENDPOINT,
...
bool test_game_is_connected(void) {
    ...
    shape shapes_connected[5 * 5] = {EMPTY, EMPTY, EMPTY,EMPTY, EMPTY,
                            EMPTY, EMPTY, EMPTY, ENDPOINT, EMPTY,
                            EMPTY, EMPTY, EMPTY, SEGMENT, EMPTY,
                            EMPTY, EMPTY, EMPTY, ENDPOINT,EMPTY,
                            EMPTY, EMPTY, EMPTY, EMPTY, EMPTY};

    direction orientations_connected[5 * 5] = {NORTH, NORTH, NORTH, NORTH, NORTH,
                                    NORTH, NORTH, NORTH, SOUTH, NORTH,
                                    NORTH, NORTH, NORTH, NORTH, NORTH,
                                    NORTH, NORTH, NORTH, NORTH, NORTH,
                                    NORTH, NORTH, NORTH, NORTH, NORTH}; 
