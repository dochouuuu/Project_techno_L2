...
bool test_game_won()
{
  ...
  // test 2 : le jeu par défaut n'est pas gagné, la solution par défaut est
  // gagnée
  ...
  // test 3 : un jeu avec 1 seule case non-vide ne peut pas être gagné (car pas
  // bien appairé)
  ...
  // test 4 : un jeu avec un petit ilot appairé et tout le reste en empty est
  // gagnant
...
bool test_game_shuffle_orientation(void)
{
  ...
  game_delete(g);
  return shuffled_00 && shuffled_11 && shuffled_22;  // S'assurer que toutes les pièces ont changé au moins
                                                     // une fois
}
