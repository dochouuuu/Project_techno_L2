...
/*Ce que fait la fonction : Crée un jeu vide de taille par défaut (DEFAULT_SIZE), avec toutes les pièces initialisées à la forme EMPTY et orientation NORTH.
Tests à implémenter :
Vérifie que le jeu est bien créé (le pointeur n'est pas NULL).
Vérifie que la taille du jeu est bien DEFAULT_SIZE.
Vérifie que toutes les pièces sont bien de forme EMPTY et orientées vers NORTH.*/
 ...
 * @return the created game*/
 /* Crée un jeu avec les formes et orientations spécifiées. Si les tableaux shapes ou orientations sont NULL, il initialise les pièces respectivement à EMPTY et NORTH.
Tests à implémenter :
Teste avec des tableaux non NULL pour les formes et orientations, et vérifie que le jeu est bien initialisé avec les valeurs données.
Teste avec des tableaux NULL, et vérifie que toutes les formes sont EMPTY et que toutes les orientations sont NORTH.
Vérifie que la taille du jeu est correcte (DEFAULT_SIZE).*/


bool test_game_new() {
    bool test = true;
    shape shapes[DEFAULT_SIZE * DEFAULT_SIZE] = {ENDPOINT, SEGMENT, EMPTY, EMPTY, EMPTY,
                                                  EMPTY, EMPTY, EMPTY, EMPTY, EMPTY,
                                                  EMPTY, EMPTY, EMPTY, EMPTY, EMPTY,
                                                  EMPTY, EMPTY, EMPTY, EMPTY, EMPTY,
                                                  EMPTY, EMPTY, EMPTY, EMPTY, EMPTY};

    direction orientations[DEFAULT_SIZE * DEFAULT_SIZE] = {NORTH, EAST, NORTH, NORTH, NORTH,
...
Tests à implémenter :
Crée un jeu, fais-en une copie et vérifie que toutes les pièces de la copie correspondent à celles du jeu original.
Modifie une pièce dans le jeu original et vérifie que la copie n'est pas affectée (vérifie que la copie est bien une nouvelle instance et non une référence partagée).*/
 ...
 **/
/*Ce que fait la fonction : Compare deux jeux pour voir s’ils sont égaux, avec une option pour ignorer l'orientation des pièces.
Tests à implémenter :
Vérifie que deux jeux identiques sont considérés comme égaux.
Vérifie que deux jeux avec des pièces différentes sont considérés comme non égaux.
Teste avec ignore_orientation à true et à false pour s'assurer que l'orientation est correctement ignorée ou prise en compte.*/
...
bool test_game_equal() {
    ...
    shape* get_random_shape_tab(void) {
        shape* tab = malloc(DEFAULT_SIZE * DEFAULT_SIZE * sizeof(shape));
    ...
    direction* get_random_dir_tab(void) {
        direction* tab = malloc(DEFAULT_SIZE * DEFAULT_SIZE * sizeof(direction));
        for (int i = 0; i < DEFAULT_SIZE * DEFAULT_SIZE; i++) {
            tab[i] = rand() % NB_DIRS;
        }
        return tab;
    ...
    shape *shape = get_random_shape_tab();
    assert(shape);
    direction *dir = get_random_dir_tab();
    assert(dir);

    // Création des jeux avec les données générées
    game g1 = game_new(shape, dir);
    game g3 = game_copy(g1);
    game g2 = game_new(shape, dir);
...
Tests à implémenter :
Appelle la fonction et vérifie qu'elle ne provoque pas d'erreurs (comme un segmentation fault).
Utilise un outil comme valgrind pour vérifier qu’il n’y a pas de fuite de mémoire après la suppression du jeu.*/
...
Teste avec des indices en dehors des bornes et vérifie que la fonction ne modifie rien ou génère une erreur.*/
...
Tests à implémenter :
Modifie l'orientation d'une pièce et vérifie que le changement a bien été effectué.
Vérifie que les autres pièces n’ont pas été affectées.
Teste avec des indices en dehors des bornes et vérifie le comportement attendu.*/
